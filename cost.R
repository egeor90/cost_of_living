...
if(length(grep(pattern=str_to_title(city_1), all_cities$city))!=0 & is.na(all_cities[grep(pattern=str_to_title(city_1), all_cities$city),"alternative"])){
  city_link_1 <- str_to_title(city_1)
  country_1 <- all_cities[grep(pattern=str_to_title(city_1), all_cities$city),"country"] %>% trimws()
}else if(length(grep(pattern=str_to_title(city_1), all_cities$city)) > 0 & length(all_cities[grep(pattern=str_to_title(city_1), all_cities$city),"alternative"])!=0){
    ...
    country_1 <- all_cities[grep(pattern=str_to_title(city_1), all_cities$city),"country"] %>% trimws()
}else if(length(grep(pattern=str_to_title(city_1), all_cities$alternative)) > 0 & length(all_cities[grep(pattern=str_to_title(city_1), all_cities$alternative),"alternative"])!=0){
...
if(length(grep(pattern=str_to_title(city_2), all_cities$city))!=0 & is.na(all_cities[grep(pattern=str_to_title(city_2), all_cities$city),"alternative"])){
  city_link_2 <- str_to_title(city_2)
  country_2 <- all_cities[grep(pattern=str_to_title(city_2), all_cities$city),"country"] %>% trimws()
}else if(length(grep(pattern=str_to_title(city_2), all_cities$city)) > 0 & length(all_cities[grep(pattern=str_to_title(city_2), all_cities$city),"alternative"])!=0){
  ...
  country_2 <- all_cities[grep(pattern=str_to_title(city_2), all_cities$city),"country"] %>% trimws()
}else if(length(grep(pattern=str_to_title(city_2), all_cities$alternative)) > 0 & length(all_cities[grep(pattern=str_to_title(city_2), all_cities$alternative),"alternative"])!=0){
...
price_2 <- as.numeric(gsub("[[:alpha:]]", "", sum_price))
...
if(country_1 == country_2){
  cat(" Date: ", format(as.Date(Sys.Date(),origin="1970-01-01")),"\n",
      ...
      paste0("You need to have ", paste(price_2, currency_2), " in ", str_to_title(city_2)," to maintain the same life in ",city_1," with ",amount_," ",currency_,"."),"\n",
    ...
    cat(" Date: ", format(as.Date(Sys.Date(),origin="1970-01-01")),"\n",
      ...
      "Compared city: ", str_to_title(city_2),"\n",
      "Amount: ", amount_, currency_,"in",str_to_title(city_1),"\n",
      "You need to have", paste0(paste(price_2, currency_2), " (",paste(price2_inbase, currency_), ") in ", str_to_title(city_2)," to maintain the same life in ",city_1," with ",amount_," ",currency_),"\n",
      "Earning rate (City 2 / City 1): ", round((price_2/price2_usd)/(as.numeric(amount_)/as.numeric(price1_usd)),2),"\n")
}
